...
const {users,loggedInUser} = useContext(SocialMediaContext)
    ...
    return(
        <div className="header-container">
            ...
            <div className="search-input-header-container">
                ...
                {
                    users.length && users.filter((user) =>inputText === ""?user._id !== loggedInUser._id && user:user.firstName.toLowerCase().includes(inputText.toLowerCase())&&user).map((user) =><div className="user-result" onClick={() => navigateTo(`/profile/${user._id}`)} key={user.firstName+user.lastName}>
